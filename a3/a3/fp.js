 ...
 * (e.g., add, sub, mul, div, rem, isLT, isGT, isZero). Some of the included
...
(function(exports){
 ...
 /** hd takes in a non-empty list and returns the first element of that list
    ...
    @alias module:fp.hd
    @return {any value}  the first element of the input list
...
/** mul takes in two numbers and returns their product
    ...
    @param n2 {number} - any finite number
    @alias module:fp.mul
    @return {number} the product of the two inputs
    @throws Exception: Both arguments of mul must be numbers.
 */
    var mul = makeArithmeticOp( "mul", function (a,b) { return a*b; } );
/** div takes in two numbers n1 and n2 and returns Math.floor(n1 / n2)
    [must be called with n1 >= 0 and n2 > 0]
    @function 
    @param n1 {number} - any finite number    
    @param n2 {number} - any finite number
    @alias module:fp.div
    @return {number} the integer division of the two inputs
    @throws Exception: Both arguments of rem must be numbers.
 */
    var div = makeArithmeticOp("div",function (a,b) {return Math.floor(a/b);} );
/** rem takes in two numbers n1 and n2 and returns n1 % n2
    [must be called with n1 >= 0 nd n2 > 0]
    @function 
    @param n1 {number} - any finite number    
    @param n2 {number} - any finite number
    @alias module:fp.rem
    @return {number} the remainder of n1 / n2
    @throws Exception: Both arguments of rem must be numbers.
 */
var rem = makeArithmeticOp( "rem", function (a,b) { return a%b; } );    
