...
(function() {
    ...
    function typeCheckPrimitiveOp(op, args, typeCheckerFunctions) {
        ...
        if (args.length !== numArgs) {
            throw new Error("Wrong number of arguments given to '" +
                    op + "'.");
        }
        ...
        switch (prim) {
            case "+":
                if (args.length < 2) {
                    ...
                    return E.createString(E.getNumValue(args[0]) +
                        E.getStringValue(args[1]));
                } else if (args[0][0] === "String" && args[1][0] === "Num") {
                    typeCheckPrimitiveOp(prim, args, [E.isString, E.isNum]);
                    return E.createString(E.getStringValue(args[0]) +
                        E.getNumValue(args[1]));
                } else {
                ...
                case ">":
                    if (E.getNumValue(args[0]) > E.getNumValue(args[1])) {
                        if (args[2][0] === "Num") {
    ...
    function interpret(source) {
        ...
        try {
            if (source === '') {
                window.alert('Nothing to interpret: you must' +
                    ' provide some input!');
            } else {
